...
use super::{add_dep, get_deps, remove_dep};
use super::{add_explicit, get_explicit, remove_explicit};
            ...
            if let Some(extension) = file_path.extension() {
                if let Some(extension_str) = extension.to_str() {
                    if extension_str == "o" || extension_str == "a" {
                        let command = format!(
                            "strip {} {}",
                            object_and_static_lib_args.join(" "),
                        ...
                        if !status.success() {
                            die!(
                                get_repo_name().as_str(),
                                "failed to strip file: {}",
                                file_path.display()
                            )
                        }
...
pub fn pkg_tar(pkg: &str) {
    ...
    create_tar_archive(tar_file.as_str(), pkg_dir.as_str(), &KISS_COMPRESS)
...
pub fn pkg_build(pkg: &str) {
    ...
    if status.success() {
        ...
        .expect("Failed to copy repository files to package directory");
